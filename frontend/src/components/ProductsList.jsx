							...
							products?.map((product) => (
								<tr key={product._id} className='hover:bg-gray-700'>
									...
									<td className='px-6 py-4 flex gap-2 whitespace-nowrap'>
										<button onClick={() => deleteProduct(product._id)} className="p-2 bg-red-100 rounded-full hover:opacity-70 transition-opacity duration-200">
											<Trash className='h-4 w-4 cursor-pointer text-red-500' />
										</button>

										<button onClick={() => handleEditProduct(product)} className="p-2 bg-blue-100 rounded-full hover:opacity-70 transition-opacity duration-200">
